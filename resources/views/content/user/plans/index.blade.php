...
// ✅ Remplace la ligne "Utilisateurs actifs" par "Vouchers/Coupons connectés"
$comparisonRows = [
  ...
  // Nouveau champ
  ['label' => 'Vouchers/Coupons connectés', 'feature' => 'vouchers_connected'],
    ...
    <div class="row g-4">
      @foreach($orderedPlans as $index => $plan)
        @php
          ...
          // ✅ Nouveau champ (avec compat rétro si anciens plans)
          $vouchersConnected = $formatLimit($features['vouchers_connected'] ?? ($features['active_users'] ?? null));
          ...
          <div class="plan-card card shadow-none">
            <div class="card-body p-4 d-flex flex-column">
              ...
              <ul class="list-unstyled plan-list flex-grow-1">
                ...
                {{-- ✅ Remplace "utilisateurs actifs" --}}
                <li><i class="icon-base ti tabler-check"></i><strong>{{ $vouchersConnected }}</strong> voucher/coupon(s) connecté(s)</li>
                        ...
                        @foreach(['starter', 'pro', 'isp'] as $tier)
                          @php
                            ...
                            if (!$plan) {
                              ...
                              // ✅ compat rétro si on tombe sur un ancien plan
                              if (($row['feature'] ?? '') === 'vouchers_connected') {
                                $display = $formatLimit($features['vouchers_connected'] ?? ($features['active_users'] ?? null));
                              } else {
