  ...
  <div class="col-md-12">
        ...
        <form id="profileAccountForm" action="{{ route('user.profile.account.update') }}" method="POST" enctype="multipart/form-data">
          ...
          <div class="d-flex align-items-start align-items-sm-center gap-4 mb-6">
            ...
            <div>
              ...
              {{-- ✅ Masqué par défaut, s'affiche seulement après sélection --}}
              <button type="button" class="btn btn-label-secondary mb-2 d-none" id="resetAvatar">
                Réinitialiser
              </button>
              ...
              {{-- ✅ Zone d'erreur JS (preview) --}}
              <div class="text-danger small mt-1 d-none" id="photoError"></div>
            </div>
          </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      function resetPreview() {
        ...
        input.value = '';
        showError('');
        showResetButton(false); // masque bouton
      }
      ...
      input.addEventListener('change', function () {
        ...
        if (file.size > maxBytes) {
          showError("Fichier trop volumineux. Taille max : 2MB.");
          resetPreview();
          return;
        }
        ...
        showResetButton(true); // affiche bouton seulement après upload
      });
      ...
      // Au chargement, toujours masqué (après save aussi car la page se recharge)
