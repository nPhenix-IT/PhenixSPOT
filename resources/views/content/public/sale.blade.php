              ...
              <div class="accordion pricing-accordion" id="passAccordion">
                <div class="accordion-item">
                  ...
                  <div id="collapseHours" class="accordion-collapse collapse show" aria-labelledby="headingHours" data-bs-parent="#passAccordion">
                    <div class="accordion-body">
                      @if($hourProfiles->where('price', '>=', 200)->isNotEmpty())
                        <div class="pricing-area">
                          @foreach($hourProfiles->where('price', '>=', 200) as $index => $profile)
                            @php
                              $commissionAmountFull = round(($profile->price * $commissionPercent) / 100, 2);
                              $commissionAmountForClient = $commissionPayer === 'client'
                                ? $commissionAmountFull
                                : ($commissionPayer === 'split'
                                  ? round($commissionAmountFull / 2, 2)
                                  : 0);

                              // ✅ affichage demandé:
                              // - client: price + full fee
                              // - split: price + half fee
                              // - seller: price
                              $displayPrice = $profile->price + $commissionAmountForClient;
                            ...
                            @include('content.public.partials.sale_profile_badge', [
                              ...
                              'commissionAmount' => $commissionAmountForClient,   {{-- ✅ fee côté client (0 / full / half) --}}
                              'commissionPayer' => $commissionPayer,
                              'displayPrice' => $displayPrice,
                              'commissionAmountFull' => $commissionAmountFull,    {{-- ✅ extra (ne casse rien si ignoré) --}}
                            ])
                          @endforeach
                        </div>
                      @else
                ...
                <div class="accordion-item">
                  ...
                  <div id="collapseData" class="accordion-collapse collapse" aria-labelledby="headingData" data-bs-parent="#passAccordion">
                    <div class="accordion-body">
                      @if($dataProfiles->where('price', '>=', 200)->isNotEmpty())
                        <div class="pricing-area">
                          @foreach($dataProfiles->where('price', '>=', 200) as $index => $profile)
                            @php
                              $commissionAmountFull = round(($profile->price * $commissionPercent) / 100, 2);
                              $commissionAmountForClient = $commissionPayer === 'client'
                                ? $commissionAmountFull
